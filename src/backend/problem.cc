...
bool Problem::AddVertex(std::shared_ptr<Vertex> vertex) {
    ...
    if (problemType_ == ProblemType::SLAM_PROBLEM) {
        if (IsPoseVertex(vertex)) {
            ResizePoseHessiansWhenAddingPose(vertex);/// 如果是位姿的话就对H_prior_,b_prior_进行扩维,加上vertex的维度,并将没有值的地方设为0(imu与cam之间也算pose)
        }
    }
    return true;
}
...
/// 对H_prior_, b_prior_进行扩维,加上v中的维度,并将没有值的地方设为0
...
//    std::cout << "pose dim: " << pose_dim <<std::endl;
    ...
    /// 前几列是pose,后几列是landmark,pose_dim是外界算好了传进来的,marg_landmark_size是刚刚算的
    /// 总之是把被marg的帧看到的landmark都要去掉了
    ...
    if(H_prior_.rows() > 0)
    {
        H_marg += H_prior_; //TODO:: 想不通这里为什么还要加这个
        b_marg += b_prior_;
    }
